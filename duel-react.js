...
(function () {
    ...
    const Z = 10008;
    let seen = { match: null, n: 0 }, lastSent = 0, open = false, wasActive = false, introT = 0;
    ...
    function _ui() {
        ...
        el.id = 'duel-react';
        el.innerHTML = `<div class="dr-label">Реакции</div><div class="dr-list">${EMOJI.map(e => `<button class="dr-e" data-e="${e}">${e}</button>`).join('')}</div><button class="dr-toggle" aria-label="Реакция">😊</button>`;
        document.body.appendChild(el);
        el.querySelector('.dr-toggle').onclick = () => { clearTimeout(introT); el.classList.remove('dr-intro'); open = !open; el.classList.toggle('dr-open', open); _h('light'); };
    ...
    //
    // 🔴 Маленькую кнопку сбоку посреди игры никто не замечает (жалоба 23.09: «а где
    // смайлики?»). Поэтому в начале КАЖДОГО матча панель сама раскрывается на 3,5 с
    // с подписью «Реакции», потом сворачивается в кнопку, которая ещё пульсирует.
    setInterval(() => {
        const active = !!(window.state && window.state.duel && window.state.duel.active);
        const el = document.getElementById('duel-react');
        if (active) {
            const ui = _ui();
            ui.classList.add('dr-on');
            if (!wasActive) {
                open = true;
                ui.classList.add('dr-open', 'dr-intro', 'dr-hint');
                clearTimeout(introT);
                introT = setTimeout(() => {
                    open = false;
                    ui.classList.remove('dr-open', 'dr-intro');
                    setTimeout(() => ui.classList.remove('dr-hint'), 6000);
                }, 3500);
            }
        } else if (el) { el.classList.remove('dr-on', 'dr-open', 'dr-intro', 'dr-hint'); open = false; }
        wasActive = active;
    }, 500);
...
.dr-label{display:none;padding:3px 8px;border-radius:var(--r-full);background:#f59e0b;color:#fff;font-size:var(--t-micro);font-weight:900;white-space:nowrap}
#duel-react.dr-intro .dr-label{display:block;animation:drIn .2s ease-out}
#duel-react.dr-hint .dr-toggle{animation:drPulse .9s ease-in-out infinite;border-color:#f59e0b}
...
@keyframes drPulse{0%,100%{transform:scale(1)}50%{transform:scale(1.18)}}

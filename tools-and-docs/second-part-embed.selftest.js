...
// 🔴 Значок показывает НАСТОЯЩЕЕ число дел второй части, а не единицу.
//
// Раньше вторая часть добавляла ровно +1 с оговоркой «точное число живёт в
// другой системе, выдумывать нельзя». Оговорка была верной, а числа не было:
// «Проверочная» отдавала только отметку «есть новое». Теперь она отдаёт сводку
// (todo — сколько ждёт сдачи, unread — сколько разборов не открыто), и значок
// обязан складывать её с домашкой первой части. Единица остаётся запасным
// путём — на первый вход, когда сводка ещё не доехала.
assert.match(uiSrc, /n \+= \(Number\(sp\.todo\) \|\| 0\) \+ \(Number\(sp\.unread\) \|\| 0\)/,
  'значок снова считает вторую часть одним делом — ученик не увидит, сколько его ждёт');
assert.match(modes, /window\.secondPartSummaryData = function\(\)/,
  'сводка второй части не отдаётся наружу — значку нечем считать');
// Непрочитанный разбор не протухает вместе с семидневным окном новостей:
// куратор писал объяснение всерьёз, и оно обязано ждать, сколько нужно.
assert.match(modes, /if \(sp && \(Number\(sp\.unread\) \|\| 0\) > 0\) return true;/,
  'раздел перестаёт гореть через неделю, даже если разбор так и не прочитан');
assert.match(bot, /unread: Number\(item\.unread\) \|\| 0/,
  'бот не переносит число непрочитанных разборов — сводка приедет неполной');

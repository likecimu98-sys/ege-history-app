...
window.updateHwNavBadge = function() {
    ...
    // Вторая часть добавляет НАСТОЯЩЕЕ число, а не единицу.
    //
    // Раньше здесь стояло `n += 1` с оговоркой «точное число живёт в другой
    // системе, выдумывать нельзя». Оговорка была верной, а вот числа не было:
    // «Проверочная» отдавала только отметку «есть новое». Теперь она отдаёт
    // сводку (todo — сколько работ ждёт, unread — сколько разборов не открыто),
    // и значок наконец говорит, сколько дел, а не просто что они есть.
    //
    // Считаем работы, а не пачки: значок должен убывать с каждой сданной
    // работой, иначе он висит одним и тем же числом весь вечер и на действие
    // не отвечает. Пачками говорят словами — в строке раздела.
    const sp = window.secondPartSummaryData ? window.secondPartSummaryData() : null;
    if (sp) n += (Number(sp.todo) || 0) + (Number(sp.unread) || 0);
    // Сводки ещё нет (первый вход, бот не донёс) — прежний путь: хотя бы
    // покажем, что там что-то есть.
    else if (window.secondPartHasNews && window.secondPartHasNews()) n += 1;

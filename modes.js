...
window.startDuelGame = function() {
    ...
    if (duelMode === 'order' && window.openOrderDuel) {
        window.state.currentMode = 'duel';
        Object.assign(window.state.duel, { active: true, myScore: 0, myCombo: 0 });
        window.openOrderDuel({
            deck: window.state.duel.orderDeck || [],
            oppName: window.state.duel.oppName,
            endsAt: (window.state.duel.startTime || Date.now()) + (window.ORDER_DUEL_MS || 45000)
        });
        return;
    }

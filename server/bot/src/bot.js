...
// 🔴 Открыто по умолчанию: класс участвует, пока его явно не закрыли.
//
// Раньше было наоборот — включали по одному, — и на деле вторая часть была
// открыта ОДНОМУ классу из четырнадцати: про тумблер забывали, а ученик видел
// «для вашего класса пока не открыта» и считал, что её нет вовсе. Тумблер
// остался, но теперь он ЗАКРЫВАЕТ, а не открывает.
async function classSecondPart(code) {
    try {
        const snap = await fdb.doc(`${base}/classes/${code}`).get();
        return !(snap.exists && snap.data() && snap.data().secondPart === false);
    } catch (e) { return true; }
}
...
async function doSecondPart(ctx) {
    ...
    await ctx.reply(
        ...
        + 'Она открыта всем вашим группам сразу — включать ничего не нужно. '
        + 'Нажмите на группу, если хотите её ЗАКРЫТЬ: раздел «Развёрнутые '
        + 'ответы» у её учеников пропадёт.\n\n'
        + 'Закрытие не удаляет уже выданное: работы и проверки остаются, '
...
bot.callbackQuery(/^sp:(.+)$/, async (ctx) => {
    ...
    try {
        ...
        await ctx.answerCallbackQuery(next ? 'Открыто' : 'Закрыто');
        await ctx.editMessageText(
            `${next ? '✅' : '▢'} «${own.name}» — вторая часть ${next ? 'открыта' : 'закрыта'}.\n\n`
            + (next
...
// («вашу работу проверили», «куратор ответил», «работу вернули») и доходит
// независимо от тумблера. Неизвестный вид считаем личным: заглушить чужое
// сообщение по ошибке хуже, чем пропустить лишнее.
const SECOND_PART_BROADCAST = new Set([
    'homework_new', 'homework_moved', 'homework_canceled', 'homework_due',
]);

// Виды, после которых у ученика во второй части появляется что-то новое.
// По ним ставится отметка в его документе — она и зажигает раздел в «Домашке».
const SECOND_PART_NEWS = new Set([
    'homework_new', 'homework_due', 'reviewed', 'answer', 'revision_requested',
]);
    ...
    const tick = async () => {
        ...
        try {
            ...
            for (const item of items.slice(0, SECOND_PART_BATCH_CAP)) {
                ...
                // 🔴 Тумблер «Домашка» гасит РАССЫЛКУ, но не личную переписку.
                //
                // Раньше через него проходило всё подряд — и «новая домашка», и
                // «вашу работу проверили», и «куратор ответил на ваш вопрос».
                // Человек выключал рассылку про задания и терял ответ на свой же
                // вопрос, ничего об этом не зная: сообщение снималось с очереди
                // как доставленное. Выключить рассылку и выключить ответ тебе
                // лично — разные желания, и второго никто не выражал.
                const kind = String(item?.kind || '');
                if (SECOND_PART_BROADCAST.has(kind)) {
                ...
                // 🔴 Отметка «во второй части есть новое» — ДО отправки.
                //
                // Сообщение в Telegram легко пролистать, а в самом приложении
                // вторая часть не подавала признаков жизни вовсе: значок на
                // вкладке «Домашка» считает только первую половину, и строка
...
// потолка Telegram (~30/с). Ставить меньше нельзя: на превышении прилетает 429 с
// flood-wait, и встаёт вся рассылка, а не одно сообщение.
const DUEL_SEND_GAP_MS = 60;
function watchDuels() {
    ...
    const attach = () => fdb.collection(`${base}/matches`).where('status', '==', 'waiting').onSnapshot(async (snap) => {
        for (const change of snap.docChanges()) {
            ...
            if (change.type === 'added') {
                ...
                const now = Date.now();
                // 🔴 ЗОВЁМ ВЕСЬ СПИСОК СРАЗУ, НО ОБРЫВАЕМ НА ПЕРВОМ ЖЕ СОПЕРНИКЕ.
                //
                // История решения. Сначала бот слал вызов всему списку — 116 сообщений на
                // матч и столько же удалений следом. 12.08.2026 это заменили волнами по 25
                // человек с паузой 12 секунд: сообщений стало 27 на матч, но последние в
                // очереди узнавали о вызове через две минуты, когда он уже протух.
                // 14.08.2026 владелец вернул сплошной обход: подписчиков 145, при
                // DUEL_SEND_GAP_MS круг занимает ~9 секунд — соперник видит вызов почти
                // сразу, и матчей должно складываться больше.
                //
                // Обрыв делает всю разницу и обязан стоять ПЕРЕД каждой отправкой: принятый
                // вызов останавливает рассылку на ближайшем же человеке, поэтому лишних
                // сообщений ровно столько, сколько успело уйти за доли секунды.
                //
                // ⚠️ Цена ковровой рассылки известна поимённо: из 325 человек 180 отключили
                // дуэли именно на ней. Поползут отказы дальше — смотреть сюда.
                //

...
test('оба определения «заданий варианта» совпадают', () => {
  // 🔴 Списков заданий домашки в сторе ДВА, и они разошлись. activeAssignmentsFor
  // (начисление баллов) собирал состав через COALESCE и знал про задания банка;
  // studentAssignments (список зачтённого, который уезжает КЛИЕНТУ) собирал его
  // только из custom_task_id, и у варианта получался массив из NULL-ов.
  // Сравнение e.task_id = ANY(...) не совпадало никогда, ни одно задание ФИПИ не
  // попадало в countedTaskIds — и приложение выдавало вариант заново. Ученик
  // решал одно и то же по кругу, а балл при этом считался верно, потому что его
  // считал ДРУГОЙ запрос. Расхождение двух определений одного и того же — самый
  // дорогой сорт ошибки: каждое место по отдельности выглядит правильным.
  const aggregation = /array_agg\(COALESCE\(at\.custom_task_id::text, at\.bank_task_id\)\)/g;
  const found = storeSource.match(aggregation) || [];
  assert.equal(found.length, 2, 'состав заданий ДЗ собирается одинаково в обоих запросах');
  const plain = storeSource.match(/array_agg\(at\.custom_task_id::text\)/g) || [];
  assert.equal(plain.length, 0, 'ни один запрос не собирает состав без заданий банка');
});

...
test('правило видимости написано ОДИН раз и применено везде', () => {
  // 🔴 Семь запросов должны отвечать одинаково: что видит ученик, что ему
  // засчитывается, кого считать в таблице учителя, чьи задания варианта
  // отдавать, по скольким людям считать сводку бота, из скольких работ
  // складывается «сдал N из M» в списке учеников и что показывает карточка
  // одного ученика. Разъехавшиеся копии одного правила уже стоили дважды.
  //
  // Само число не догма: появился ещё один запрос, который спрашивает «видит
  // ли ученик эту работу», — подставьте ту же константу и поправьте счёт. Тест
  // ловит другое: копию правила, написанную рядом своими словами.
  ...
  assert.equal(uses, 7, `правило подставлено в семь запросов, а не в ${uses}`);
});

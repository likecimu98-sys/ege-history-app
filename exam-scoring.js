...
(function(root, factory) {
    ...
    // ⚠️ Здесь был баг, из-за которого КАЖДЫЙ верный ответ помечался «засчитано с
    // предупреждением» (26.07.2026). Обе проверки смотрели ТОЛЬКО на ввод ученика и
    // ни разу не сравнивали его с эталоном:
    //   `case`    — срабатывал, если во вводе есть хоть одна строчная буква. То есть
    //               на «Сталинград» и вообще на любом русском слове.
    //   `spacing` — срабатывал, если во вводе есть хоть один неалфавитный символ. То
    //               есть на «Ярослав Мудрый» и «Санкт-Петербург», где пробел и дефис
    //               стоят и в самом эталоне.
    // Цена ошибки была не косметическая: acceptedWithWarning красит ответ как
    // неверный, показывает «на реальном ЕГЭ так нельзя» и кладёт задание в разбор
    // ошибок. Ученик видел свой ответ, ровно совпадающий с эталоном, и подпись, что
    // он неправильный.
    //
    // Проверка `case` убрана совсем: регистр на ЕГЭ не критерий, эксперт засчитывает
    // ответ в любом. Совет про печатные буквы бланка — это про заполнение бланка, а
    // не про верность ответа, и ему место в обучалке, а не в оценке каждой строки.
    function textWarnings(actual, matchedRaw, typoDistance) {
        ...
        // Сравниваем ТОЛЬКО скелет разделителей: выкидываем буквы и цифры, остаётся
        // строка из пробелов и знаков. Так «пропущенный дефис» ловится, а опечатка в
        // букве не превращается заодно в мнимую претензию к пробелам.
        //
        // ⚠️ Перед сравнением скелет НОРМАЛИЗУЕТСЯ. Без этого точка в конце ответа,
        // запятая, кавычки, двойной или неразрывный пробел давали ровно то же
        // предупреждение, что и по-настоящему значимый пропущенный дефис, — то есть
        // полностью верный ответ красился как небрежный, попадал в работу над
        // ошибками и не выходил из ротации пробника.
        // На бланке ЕГЭ знаки препинания не вводят ВООБЩЕ: это артефакт свободного
        // ввода в приложении, а не критерий проверки. Значимо только различие
        // ВНУТРИ слова — пропущенный дефис в «Санкт-Петербург».
        const punct = value => String(value)
            .replace(/[0-9A-Za-zА-Яа-яЁё]/g, '')      // остаётся скелет разделителей
            .replace(/[   \t]/g, ' ')  // неразрывные пробелы — обычные
            .replace(/[«»""„‟''‚‛"'`]/g, '')          // кавычки — оформление, не смысл
            .replace(/\s+/g, ' ')                     // повторяющиеся пробелы схлопываем
            .replace(/[.,;:!?…]+/g, '')               // знаки препинания на бланк не вводят
            .trim();
        if (punct(raw) !== punct(expected)) warnings.push('spacing');

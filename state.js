...
function unifyTask7Applies(rows) {
    ...
    const GROUPS = [
        // Обобщение «создан в XI в.» истинно не только для двух Софийских
        // соборов, но и для произведений/рукописей XI века. Без общей группы
        // узкий режим мог поставить рядом Софийский собор и «Житие Бориса
        // и Глеба», после чего характеристика подходила к обоим.
        [/памятник культуры создан в (?:xi|11) в/i, [2, 3, 4, 5, 6, 166]],
        // «Создан при Ярославе Мудром» относится и к софийским соборам, и
        // к «Слову о Законе и Благодати», написанному в его княжение.
        [/(?:создан[а-я]* при князе|возведен[а-я]* в годы княжения) ярослав[а-я]* мудр/i, [2, 3, 4]],
        // Софийский собор в Новгороде и Спас на Нередице одинаково защищают
        // ответ «построен в Новгородской земле».
        [/построен[а-я]* в новгородск[а-я]* земл/i, [3, 14]],
        // Нестор / монах Киево-Печерского монастыря — защитимый автор для
        // летописи и обоих наименований жития Бориса и Глеба.
        [/(?:нестор|монах[а-я]* киево-печерск[а-я]* монастыр)/i, [6, 11, 166]],
        ...
        // «Поучение детям», «Повесть временных лет» и «Слово о полку Игореве»
        // действительно созданы в XII в. Общая датировка одного не должна
        // становиться вторым правильным ответом для другого.

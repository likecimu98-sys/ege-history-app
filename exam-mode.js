...
(function() {
    ...
    const BANK_SRC = 'exam-bank.generated.js?v=20260727-1';
    ...
    // Парсер ФИПИ иногда разрывает годы пробелами внутри числа: «18 10 г.», «1 87 0- е гг.».
    // Чиним ТОЛЬКО чистые год-токены (после схлопывания пробелов строка обязана быть
    // годом с необязательным «-е» и «г./гг.») — обычные варианты с цифрами не трогаем.
    const YEAR_TOKEN_RE = /^(\d{3,4})(-?[ех])?(гг?\.)?$/;
    function normalizeOptionText(text) {
        const raw = String(text == null ? '' : text).replace(/ /g, ' ');
        const squashed = raw.replace(/\s+/g, '');
        const m = YEAR_TOKEN_RE.exec(squashed);
        if (!m) return raw;
        const decade = m[2] ? '-' + m[2].replace(/^-/, '') : '';
        return m[1] + decade + (m[3] ? ' ' + m[3] : '');
    }

    function optionText(task, digit) {
        return normalizeOptionText(optionForDigit(task, digit)?.text || String(digit || ''));
    }
    ...
    function orderedPoolHtml(task, slots, readonly) {
        ...
        return `<div class="em-options">${items.map(item => {
            ...
            return `<button class="dnd-chip em-dnd-chip ${selected ? 'selected' : ''}" ${readonly ? 'disabled' : `data-em-action="chip" data-value="${digit}"`}>${escapeAttr(normalizeOptionText(item.text))}</button>`;
    ...
    function renderOrderedTask(task, value, readonly, scoreResult) {
        const slots = normalizedOrderedAnswer(task, value);
        const prompt = task.kim === 2
            ? '<div class="em-task-prompt">Расположите в хронологической последовательности исторические события — от самого раннего к самому позднему.</div>'
            : '';
        ...
        return `<div class="em-classic-task"><section class="em-board-card">${prompt}${orderedBoardHtml(task, slots, readonly)}</section><aside class="em-pool-panel">${panel}${panelActions(readonly, task, scoreResult)}</aside></div>`;
    }
    ...
    function choiceOptionsHtml(task, value, readonly) {
        ...
        return `<div class="em-options">${(task.elements || []).map(item => {
            const digit = String(item.n);
            return `<button class="dnd-chip em-dnd-chip em-choice-chip ${selected.has(digit) ? 'selected' : ''}" ${readonly ? 'disabled' : `data-em-action="choice" data-value="${digit}"`}><span class="em-choice-number">${digit}</span><span>${escapeAttr(normalizeOptionText(item.text))}</span></button>`;
        }).join('')}</div>`;
    }
